...
export default function WeddingGallery() {
  ...
  const filteredItems = slideData.filter((item) => item.category === selectedCategory);
  ...
  const handleSlideClick = (index: number) => {
    ...
    setModalImage({ src: item.src, title: item.title, category: item.category });
    setIsModalOpen(true);
  };
  ...
  const goPrev = () => {
    ...
    setModalImage({ src: filteredItems[newIndex].src, title: filteredItems[newIndex].title, category: filteredItems[newIndex].category });
  };
    ...
    <section className="py-20 px-4 sm:px-6 lg:px-10 transition-colors duration-300" id="gallery">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-4">
          <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-md font-medium mb-3 text-[#c7a47a] tracking-wide uppercase">
            Galeri
          </motion.p>
          <motion.h2
        ...
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          <DialogContent showCloseButton={false} className="fixed z-[999] flex items-center justify-center bg-black/45 py-10 border-none min-w-full min-h-screen rounded-none">
            ...
            <div className="absolute top-8 right-6 flex gap-12">
              ...
              <a href={modalImage?.src} download className="text-white hover:scale-110 transition">
                {" "}
